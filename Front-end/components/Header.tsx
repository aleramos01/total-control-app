...
const Header: React.FC<HeaderProps> = ({ brandSettings, user }) => {
  ...
  // Measure the real header height after each state change and on mount
  useEffect(() => {
    const measure = () => {
      if (headerRef.current) setSpacerHeight(headerRef.current.offsetHeight);
    };
    measure();
    // Re-measure after transition finishes (300ms)
    const id = setTimeout(measure, 320);
    return () => clearTimeout(id);
  }, [scrolled]);

  useEffect(() => {
    // Use hysteresis: collapse only well past the full header, expand only near top.
    // This prevents the scroll-position feedback loop that occurs when a sticky header
    // changes height — with `fixed` positioning the loop cannot happen at all, but
    // hysteresis is kept as a safety net.
    const onScroll = () => {
      const y = window.scrollY;
      setScrolled(prev => {
        if (!prev && y > 280) return true;
  ...
  return (
    <>
      {/*
        Spacer reserves the exact space the fixed header occupies in the document flow.
        It transitions alongside the header so content never jumps.
      */}
      <div
        className={`transition-all duration-300 ${scrolled ? 'mb-3' : 'mb-6'}`}
      ...
      <header
        ref={headerRef}
        className={`fixed top-6 z-30 will-change-transform border border-white/10 bg-slate-800 transition-all duration-300 ${
          scrolled
            ? 'rounded-2xl px-5 py-2.5 shadow-[0_4px_24px_rgba(15,23,42,0.5)]'
            : 'rounded-[28px] px-5 py-6 shadow-[0_20px_70px_rgba(15,23,42,0.45)]'
        }`}
        style={{
          // Mirror the parent container's horizontal constraints (max-w-5xl mx-auto px-4 sm:px-6)
          left: 'max(1rem, calc(50% - 40rem + 1rem))',
          right: 'max(1rem, calc(50% - 40rem + 1rem))',
        }}
      >
        {scrolled ? (
          /* ── Compact bar ── */
